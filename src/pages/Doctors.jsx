...
const Doctors = () => {
  ...
  return (
    <div>
        ...
        <button className='md:hidden bg-blue-500 mt-4 text-white text-sm font-light px-14 py-3 rounded-2xl'>Filters</button>
        <div className='flex flex-col sm:flex-row items.start gap-5 mt-5'>
         ...
         <div style={{ display: 'grid' , gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }} className='w-full grid-cols-4 gap-4 gap-y-6 '>
          {
            filterDoc.map(
              (item,index)=>(
                <div onClick={()=>navigate(`/appointment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all durtaion-500'>
